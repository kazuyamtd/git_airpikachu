class RoomsController < ApplicationController
  ...
  def preload
    today = Date.today
    reservations = @room.reservations.where("(start_time >= ? OR end_date >= ?) AND status = ?", today, today, 1)
  ...
  def preview
    ...
    output = {
      conflict: is_conflict(start_time, end_date, @room)
    }
  ...
  private
    def is_conflict(start_time, end_date, room)
      check = room.reservations.where("(? < start_time AND end_date < ?) AND status = ?", start_time, end_date, 1)
